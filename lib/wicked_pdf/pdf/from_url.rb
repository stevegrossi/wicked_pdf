class PDF
  class FromURL
    ...
    def initialize(url, options)
      ...
      command = [find_wkhtmltopdf_binary_path]
    ...
    def find_wkhtmltopdf_binary_path
      '/usr/local/var/rbenv/shims/wkhtmltopdf'
      ...
      #   nil
      # end
      # exe_path ||= possible_locations.map { |l| File.expand_path("#{l}/#{WickedPDF::EXE_NAME}") }.find { |location| File.exist?(location) }
      # exe_path || ''

      # fail "Location of #{WickedPDF::EXE_NAME} unknown" if @exe_path.empty?
      # fail "Bad #{WickedPDF::EXE_NAME}'s path: #{@exe_path}" unless File.exist?(@exe_path)
      # fail "#{WickedPDF::EXE_NAME} is not executable" unless File.executable?(@exe_path)
    end
  end
end
